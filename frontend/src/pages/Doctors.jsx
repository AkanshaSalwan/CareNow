  ...
  return (
    <div>
      ...
      <div className='flex flex-col sm:flex-row items-start gap-5 mt-5'>
        <button onClick={() => setShowFilter(!showFilter)} className={`py-1 px-3 border rounded text-sm  transition-all sm:hidden ${showFilter ? 'bg-[#3ED2D1] text-white' : ''}`}>Filters</button>
        ...
        <div className='w-full grid grid-cols-2 sm:grid-cols-auto gap-4 gap-y-6'>
          {filterDoc.map((item, index) => (
            <div
              onClick={() => { navigate(`/appointment/${item._id}`); scrollTo(0, 0) }}
              className='border border-[#C9D8FF] rounded-xl overflow-hidden cursor-pointer bg-white flex flex-col h-full hover:-translate-y-2 transition-all duration-300'
              key={index}
            >
              <img className='w-full h-40 object-cover bg-[#EAEFFF]' src={item.image} alt="" />
